...
class ToPoincare(nn.Module):
    ...
    Also implements clipping from https://arxiv.org/pdf/2107.11472.pdf (CVPR 2022)

    for the work: https://arxiv.org/abs/2203.10833 (CVPR 2022)
    last = hypnn.ToPoincare(
            c=0.1,
            ball_dim=128,
            riemannian=False,  # do not use riemannian to optimize hyperbolic operations ??
            clip_r=2.3,  # use clip_r.
        )
    """

    def __init__(self,
                 c,  # 1.0
                 train_c=False,  # store_true, default: False
                 train_x=False,  # store_true, default: False
                 ball_dim=None,  # 2
                 riemannian=True,
                 clip_r=None):
        ...
        self.clip_r = clip_r
        # self.norm = nn.LayerNorm(ball_dim)  # use LayerNorm to replace Clip_r.
